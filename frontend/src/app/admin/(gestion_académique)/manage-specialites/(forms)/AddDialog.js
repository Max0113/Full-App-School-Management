...
export function AddDialog({ open, onOpenChange, refresh, setrefresh }) {
  ...
  const onSubmit = async (data) => {
    ...
    setError(false);
    try {
      await Connect_Speialite.addspeialite(data);
      onOpenChange(false);
      toast.success("Speialite created", {
        description: `${data.name} Speialite has been added successfully.`,
      });
    } catch (error) {
      const message =
      ...
      setError(message);
      toast.error("Couldn't create Speialite", {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-full sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Add New Speialite</DialogTitle>
          <DialogDescription>
            Chose Name of year and click in add Speialite
          </DialogDescription>
        </DialogHeader>
