...
export function EditDialog({ level, open, onOpenChange, refresh, setrefresh }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await Connect_Level.Updatelevel(payload);
      onOpenChange(false);
      toast.success("Level updated", {
        description: `${data.name} level has been updated successfully.`,
      });
    } catch (error) {
      const message =
        error?.response?.data?.message || "Failed to update level info.";
      setError(message);
