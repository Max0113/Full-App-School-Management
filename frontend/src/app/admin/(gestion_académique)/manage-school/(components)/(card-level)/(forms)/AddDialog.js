...
export function AddDialog({ open, onOpenChange, refresh, setrefresh }) {
  ...
  const onSubmit = async (data) => {
    ...
    setError(false);
    try {
      await Connect_Level.addlevel(data);
      onOpenChange(false);
      toast.success("Level created", {
        description: `${data.name} Level has been added successfully.`,
      });
    } catch (error) {
      const message =
      ...
      setError(message);
      toast.error("Couldn't create level", {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-full sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Chose Name of year and click in add level
          </DialogDescription>
        </DialogHeader>
