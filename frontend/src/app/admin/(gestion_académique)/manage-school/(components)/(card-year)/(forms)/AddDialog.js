...
export function AddDialog({ open, onOpenChange, refresh, setrefresh }) {
  ...
  const onSubmit = async (data) => {
    ...
    setError(false);
    try {
      await Connect_SchoolYear.addschoolyear(data);
      onOpenChange(false);
      toast.success("School year created", {
        description: `${data.name} year has been added successfully.`,
      });
    } catch (error) {
      const message =
      ...
      setError(message);
      toast.error("Couldn't create school year", {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-full sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Chose Name of year and click in add School year
          </DialogDescription>
        </DialogHeader>
