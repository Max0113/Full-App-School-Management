...
export function EditDialog({ level, open, onOpenChange, refresh, setrefresh }) {
  ...
  const onSubmit = async (data) => {
    ...
    setError(false);
    try {
      const payload = { ...data, id: level.id };
      if (!payload.password) delete payload.password;

      await Connect_SchoolYear.Updateschoolyear(payload);
      onOpenChange(false);
      toast.success("School Year updated", {
        description: `${data.name} Year has been updated successfully.`,
      });
    } catch (error) {
      const message =
        error?.response?.data?.message || "Failed to update Year info.";
      setError(message);
      toast.error("Couldn't update School Year", {
        description: "Failed to update Year info.",
      });
    } finally {
      setSubmitting(false);
      route.refresh();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-full sm:max-w-lg">
        ...
        >
          <Field>
            <Label htmlFor="name">School Year</Label>
            <Input
              id="name"
              type="text"
              placeholder="e.g. Bac"
