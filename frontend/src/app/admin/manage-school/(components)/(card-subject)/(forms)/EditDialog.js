  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast.success("Subject updated", {
        description: `${data.name} subject has been updated successfully.`,
      });
    } catch (error) {
      const message =
        error?.response?.data?.message || "Failed to update Subject info.";
      setError(message);
      toast.error("Couldn't update Subject", {
        description: "Failed to update subject info.",
      });
    } finally {
      setSubmitting(false);
      route.refresh();
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-full sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the Subject's details, then click "Save changes" to confirm.
          </DialogDescription>
        </DialogHeader>
        ...
        >
          <Field>
            <Label htmlFor="name">Name Level</Label>
            <Input
              id="name"
              type="text"
              placeholder="e.g. Bac"
